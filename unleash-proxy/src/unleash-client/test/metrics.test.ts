...
const nockMetrics = (url: string, code = 200) =>
    nock(url).post(metricsUrl).reply(code, '');
const registerUrl = '/client/register';
const nockRegister = (url: string, code = 200) =>
...
test('registerInstance, sendMetrics, startTimer and count should respect disabled', (t) =>
    new Promise((resolve) => {
        ...
        Promise.all([metrics.registerInstance(), metrics.sendMetrics()]).then(
            (results) => {
...
test('should send content-type header', (t) =>
    new Promise((resolve) => {
        const url = getUrl();
        t.plan(2);
        const metricsEP = nockMetrics(url).matchHeader(
            'content-type',
            'application/json',
        );
        const regEP = nockRegister(url).matchHeader(
            'content-type',
            'application/json',
        );
